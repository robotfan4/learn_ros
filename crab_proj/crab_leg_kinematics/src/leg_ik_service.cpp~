// parase the KDL tree and get the kinematic chain for every leg, publish the ik service
// modified for phantomX robot at 2017
// author: fan {robot.fan4@gmail.com}
...
bool LegKinematics::init() {
    ...
    node_private.param("root_name", root_name, std::string("body_link"));
    node_private.param("tip_name", tip_name, std::string("foot"));
    ...
    ik_service = node_private.advertiseService("/crab_leg_kinematics/get_ik",&LegKinematics::getLegIKSolver,this);
...
bool LegKinematics::loadModel(const std::string xml) {
    ...
    for (int i=0; i<num_legs; i++){
        ...
        // parase the tree through root and tip to get the chain
