...
float x=120*sqrt(3), y=-120,z=-120;
typedef boost::shared_ptr<visualization_msgs::InteractiveMarkerFeedback const> xyzfeedbackConstPtr;

void xyzcallback(const xyzfeedbackConstPtr& state){
...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber sub=node.subscribe("/basic_controls/feedback",1,xyzcallback);
...
//	ik_solver_vel -> setWeightTS(matrix_Mx);
	...
	int ik_valid;
   // double x_0 = 120*sqrt(3), y_0 = -120, z_0 = -120, x, y, fi;  //0.14208; -0.14555; -0.11145
	while (ros::ok()){
//		if (fi > 6.28) fi = 0;
//        x = x_0 + 50 * cos(fi);
//        y = y_0 + 50 * sin(fi);
