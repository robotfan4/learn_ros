...
bool BodyKinematics::init() {
    ...
    node.param("root_name", root_name, std::string("leg_center"));
    node.param("tip_name", tip_name, std::string("coxa"));
    ...
    bs.leg_radius = 0.141;
    bs.z = -0.12;
    // this body parameters should change
...
bool BodyKinematics::loadModel(const std::string xml){
    ...
    //Vector frame= root_frame*tipframe
	for (int i=0; i<num_legs; i++){
